...
fun eval_prog (e,env) =
  let fun shift (dx, dy, exp) =
        case exp of
            NoPoints => e
          | Point (x, y) => Point(x + dx, y + dy)
          | Line (m, b) => let val new_b = dy + b - (m * dx)
                           in Line(m, new_b)
                           end
  ...
  in
	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      | Shift(x,y,exp) => shift(x,y,eval_prog(exp, env))
  end

(* geom_exp -> geom_exp
   Produce a preprocessed expression where any LineSegment has either
   dx or dy > 0.00001. Any failure becomes a Point. LineSegments are then 
   ordered such that x1 < x2. If x1 = x2, then y1 < y2. *)
